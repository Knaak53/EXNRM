...
function loadTabContent(tabName, charData, clothesData, identityData) {
    $.get('pages/' + tabName + '.html', function(data) {
        ...
        if (tabName == 'style') {
            ...
            <option value="12">Penumbra</option>
            <option value="13">Sombra grisacea</option>
            <option value="14">Tequila Sunrise</option>
            <option value="15">Atomico</option>
            <option value="16">Warp</option>
            <option value="17">Ranger espacial</option>
            <option value="19">Ying Yang</option>
            <option value="20">Bullseye</option>
            <option value="21">Lagarto</option>
            <option value="22">Dragon</option>
            <option value="23">Extraterrestre</option>
            <option value="24">Cabra</option>
            <option value="25">Sonriente</option>
            <option value="26">Poseido</option>
            <option value="27">Demonio</option>
            <option value="28">Infectado</option>
            <option value="29">Alien</option>
            <option value="30">No Muerto</option>
        ...
        else if ((tabName == 'accessories') && clothesData) {
            let male = (charData.sex == 0)
            loadAccesoriesTab(tab, clothesData, male)
        }else if ((tabName == 'masks') && clothesData) {
            let male = (charData.sex == 0)
            loadMasksTab(tab, clothesData, male)
        }
...
function loadOptionalIdentity(element, identityData) {
    ...
    $.get('pages/optional/esxidentity.html', function(data) {
        ...
        pdata.find('.name').each(function() {
            $(this).prop('placeholder', 'Maximo. ' + identityLimits.namemax + ' Caracteres');
        });
...
function loadAccesoriesTab(tab, clothesData, male) {
...
function loadMasksTab(tab, clothesData, male) {
...
$('#main .menuclose').on('click', function(evt) {
    ...
    let message = 'Seguro que quieres ' + action + ' tus cambios y salir?';
    let popupData = { 
        title: 'Confirmar', 
        message: message
    };
...
$(document).on('click', 'input:radio[name=sex]', function(evt) {
    if(radioChecked == false)
    {
        let popupData = {
            title: 'Confirmar', 
            message: 'Cambiar el sexo de tu personaje hará que pierdas tu personalización actual. Seguro que quieres hacerlo?'
        };
...
$(document).on('refresh', 'input[type=range]#height', function(evt) {
    let cm = $(this).val();
    let metros = Math.floor(cm / 100);
    let cmLeft = cm % 100;
    if (cmLeft.toString().length == 1) {
        cmLeft = '0' + cmLeft;
    }

    let valueCenter = $(this).parent().siblings('.valuelabel.center');
    valueCenter.text(metros + 'm ' + cmLeft + 'cm');
