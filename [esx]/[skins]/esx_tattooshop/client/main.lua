...
RegisterNetEvent('esx_tattooshop:openMenu_ui')
AddEventHandler('esx_tattooshop:openMenu_ui', function()
	Citizen.Wait(250)
	print("hola")
	OpenShopMenu()
end)
...
DeleteTattoo = function()
    for i=1, #pedEntities do
        local tattoo = pedEntities[i]
        if DoesEntityExist(tattoo) then
            DeletePed(tattoo)
            SetPedAsNoLongerNeeded(tattoo)
        end
    end
end
...
Citizen.CreateThread(function()
	for i = 1, #Config.Shops do
        hash = GetHashKey('u_m_y_tattoo_01')
        RequestModel(hash)
