...
RegisterNetEvent("esx_people_interactions:carry")
AddEventHandler("esx_people_interactions:carry", function(entity)
	if not carry.InProgress then
		local closestPlayer = ESX.Game.GetPlayerServerIdFromPlayerEntity(entity)
		if closestPlayer == -1 then
		    ESX.ShowNotification("~r~No hay nadie cerca a quien cargar!")
		else
		    print(targetSrc)
			if closestPlayer ~= -1 then
				carry.InProgress = true
				carry.targetSrc = closestPlayer
				TriggerServerEvent("CarryPeople:sync", closestPlayer)
