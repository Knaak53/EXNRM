...
function SendObjectData()
    ...
    if entity.type == 0 then
        ESX.ShowNotification("~r~El objeto seleccionado no tiene acciones disponibles...")
        return
    end
...
function getActionList()
    if entity.type == 1 then -- peds
        if entity.isPlayer then 
            return getPlayerActions(entity.isSelf)
        else
            return getNpcActionList()
        end
    elseif entity.type == 2 then -- Coches
        return getVehiclesActionList()
    elseif entity.type == 3 then -- props
        return getObjectsActionList()
    end
end

function getPlayerActions(isSelf)
    local ret = {}
    local configToPass
    if isSelf then
        configToPass = Config.selfActions
    else
        configToPass = Config.playerActions
    end
    local playerActionsList = getActionListFromConfigList(configToPass)
    if #playerActionsList.actions > 0 or not playerActionsList.tooFar and not playerActionsList.foundBlockedAction then
        ret = getSubMenusAndButtonsFromActionList(playerActionsList.actions)
        return true, ret
    else
        if #playerActionsList.actions <= 0  then
            ESX.ShowNotification("~r~No hay acciones disponibles!")
            return false, nil
        elseif playerActionsList.tooFar then
            ESX.ShowNotification(tooFarAway)
            return false, nil
        elseif playerActionsList.foundBlockedAction then
            ESX.ShowNotification("~r~No Puedes interactuar con esa persona!")
            return false, nil
        end
    end
end
...
function getObjectsActionList()
    ...
    elseif(objectActionsList.tooFar) then
        ...
        local genericObjectsAction = getActionListFromConfigList(Config.genericObjectActions)
        if #genericObjectsAction.actions > 0 and not genericObjectsAction.tooFar and not genericObjectsAction.foundBlockedAction then
            local ret = getSubMenusAndButtonsFromActionList(genericObjectsAction.actions)
            return true, ret
        else
            if genericObjectsAction.tooFar then
                ESX.ShowNotification(tooFarAway)
                return false, nil
            elseif genericObjectsAction.foundBlockedAction then
                ESX.ShowNotification("~r~No puedes interactuar con ese objeto!")
                return false, nil
            end 
        end
    end
...
function getVehiclesActionList()
    local vehicleActions = {}
    local foundBlockedAction = false
    local tooFar = false
    local check = false
    for k,v in pairs(Config.genericVehicleActions) do
        if Config.possibleVehicleActions[k] == nil then
            v.models = {"generic"}
            Config.possibleVehicleActions[k] = v
        end
    end
    for k,v in pairs(Config.possibleVehicleActions) do
        for j,y in pairs(v.models) do
            if type(y) == "string" and y ~= "generic" then
                y = GetHashKey(y)
            end
            if GetEntityModel(entity.target) == y or y == "generic" then
                if not v.restrictedJobs or (v.restrictedJobs and currentJobAllowAction(v.restrictedJobs)) then
                    if not v.whiteListedJobs or (v.whiteListedJobs and currentWhiteListedJobs(v.whiteListedJobs)) then
                        if not v.entityCoords or (v.entityCoords and v.entityMaxDistanceRadius and #(entity.coords - v.entityCoords) < v.entityMaxDistanceRadius) then
                            if #(entity.coords - playerCoords) < v.actionDistance then
                                
                                table.insert(vehicleActions, {label = v.label, action = k, subMenu = v.subMenu})
                            else
                                tooFar = true
                                foundBlockedAction = true
                                --print("F distancia")
    ...
    if #vehicleActions > 0 then
        local finalVehicleActions = getSubMenusAndButtonsFromActionList(vehicleActions)
        return true, finalVehicleActions
    end
    if foundBlockedAction then
        if tooFar then
            ESX.ShowNotification(tooFarAway)
            return false, nil
        else
            ESX.ShowNotification("~r~No puedes interactuar con ese vehiculo!")
            return false, nil
        end   
    end
end
...
function checkForSpecificOptions(action)
    ...
    elseif entity.type == 2 and Config.possibleVehicleActions[action] then
        Config.possibleVehicleActions[action].action(entity, action)
        return true
...
function checkForGenericOptions(action)
    for k,v in pairs(Config.genericObjectActions) do
        if k == action then
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(1, z_key) and not display then
            display = not display
            SetDisplay(display)
        end
        if IsControlJustReleased(1, z_key) and display then
            display = false
            SetDisplay(display)
        end
        if display then
