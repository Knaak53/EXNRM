...
function RefreshBussHUD()
	...
	if ESX.PlayerData.job.grade_name == 'boss' then
		...
		ESX.TriggerServerCallback('esx_society:getSocietyMoney', function(money)
			print("money de la sociedad:"..money)
...
function OpenBossMenu(society, close, options)
	...
	ESX.TriggerServerCallback('esx_society:isBoss', function(result, mon)
		isBoss = result
	end, society)
	...
	--table.insert(elements, {label = _U('show_society_money'), value = 'show_society_money'})


	if society_money then
		table.insert(elements, {label = '<b><span style="color:green;"> Fondos: '..society_money..' $</span></b>', value =  'show_society_money'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society, {
		...
		elseif data.current.value == 'show_society_money' then
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			elseif data.value == 'fire' then
				...
				ESX.TriggerServerCallback('esx_society:setJob', function()
					OpenEmployeeList(society)
				end, employee.identifier, 'desempleado', 0, 'fire')
			end
		end, function(data, menu)
			menu.close()
			OpenManageEmployeesMenu(society)
				...
				if data2.current.value == 'yes' then
					ESX.TriggerServerCallback('esx_society:getJob', function(job)
						...
						ESX.TriggerServerCallback('esx_society:setJob', function()
							OpenRecruitMenu(society)
							print(#job.grades - 1)
						end, data.current.identifier, society, #job.grades - 1, 'hire')
...
function OpenManageGradesMenu(society)
	...
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		...
		for k,v in pairs(job.job_grades) do
			local gradeLabel = (v.label == '' and job.label or v.label)

			table.insert(elements, {
				label = ('%s - <span style="color:green;">%s</span>'):format(gradeLabel, _U('money_generic', ESX.Math.GroupDigits(v.salary))),
				value = v.name
			})
		end
