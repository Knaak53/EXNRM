...
local zonesBlips = {
    {coords = vector3(62.04, -1899.48, 21.66), name = "Barrio Resicendial", color = 0},
    {coords = vector3(1261.8, -1744.04, 49.48), name = "Suburbios del Este", color = 15},
    {coords = vector3(1121.24, -560.39, 55.74), name = "Urbanizacion de Mirror Park", color = 26},
    {coords = vector3(-319.97, 478.57, 111.99), name = "Vinewood Hills Este", color = 46},
    {coords = vector3(-1084.61, 623.91, 111.99), name = "Vinewood Hills Oeste", color = 27},
    {coords = vector3(-252.17, 6387.2, 111.99), name = "Viviendas de Paleto Bay", color = 12},
    {coords = vector3(1836.31, 3848.96, 111.99), name = "Caravanas de Sandy Shores", color = 62}
}

Citizen.CreateThread(function()
    for i = 1, #zonesBlips do
        local blip = AddBlipForCoord(zonesBlips[i].coords)
        SetBlipSprite(blip, 374)
        SetBlipColour(blip, zonesBlips[i].color)
        SetBlipAsShortRange(blip, true)
        SetBlipScale(blip, 0.8)
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString(zonesBlips[i].name)
...
Citizen.CreateThread(function()
    while OwnedHouse == nil do Wait(500) end
    while true do
        Wait(0)
...
function openOutfitsMenu()
    local elements = {
        {label = "Tus Outfits", value = 'player_dressing'},
        {label = "Borrar Outfit", value = 'remove_cloth'}
    }
        ...
        elseif data.current.value == 'remove_cloth' then
            ...
            ESX.TriggerServerCallback('esx_property:getPlayerDressing', function(dressing)
                ...
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'remove_cloth', {
                    ...
                    ESX.ShowNotification("Has eliminado el Outfit")
                end, function(data2, menu2)
                    menu2.close()
                end)
            end)
            ...
            if IsControlJustReleased(0, 38) then
                ...
                function(data, menu)
                    if data.current.value == "inventory" then
                        menu.close()
                        Citizen.Wait(150)
                        TriggerEvent('esx_generic_inv_ui:openHouse')
                    elseif data.current.value == "outfits" then
                        menu.close()
                        openOutfitsMenu()
                    end
                end, 
