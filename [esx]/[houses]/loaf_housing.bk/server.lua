            ...
            MySQL.Async.fetchAll("SELECT * FROM bought_houses WHERE houseid=@houseid", {['@houseid'] = id}, function(result)
                ...
                if not result[1] then
                    if xPlayer.getAccount('bank').money >= Config.Houses[id]['price'] then
                        ...
                        MySQL.Sync.execute("UPDATE datastore_data set data = @data where name = @name and owner = @owner", {['data'] = '{"inventory": {"items": [], "weapons": [], "accounts": []}}', ['name'] = 'housing', ['owner'] = xPlayer.identifier})
                    else
