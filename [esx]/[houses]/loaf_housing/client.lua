...
local blips = {}
local Knockings = {}
local found, coords, heading, parksended
local furnicing = false
local cursorModeOn = false
Citizen.CreateThread(function()
    while ESX == nil do ESX = exports.extendedmode:getSharedObject() Wait(0) end
            ...
            if Vdist2(GetEntityCoords(PlayerPedId()), v['door']) <= 2.5 then
                ...
                while Vdist2(GetEntityCoords(PlayerPedId()), v['door']) <= 2.5 do
                    if OwnedHouse.houseId == k then
                        ...
                        --print(k)
                        --print(json.encode(AvailableHouses))
                        if AvailableHouses[k] then
                            --print("my house:".. OwnedHouse.houseId)
                            if OwnedHouse.houseId ~= 0 and OwnedHouse.houseId ~= nil then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if OwnedHouse.houseId == k then
                            ...
                            ESX.UI.Menu.Open(
                                ...
                                function(data, menu)
                                    if data.current.value == 'enter' then
                                        --print ("No deja entrar por que la casa la ha pagado otro")
                                    ...
                                    elseif data.current.value == 'garage' then
                                        ...
                                        if found then
                                            TriggerEvent("esx_advancedgarage:OpenCarsMenu", coords)
                                        end
                                        --local found, coords, heading = GetClosestVehicleNodeWithHeading(coords.x, coords.y, coords.z, 3.0, 100.0, 2.5)
                                        --if found then
                                            --ESX.UI.Menu.CloseAll()
                            ...
                            if AvailableHouses[k] and (OwnedHouse.houseId == 0 or OwnedHouse.houseId == nil) then
                                ESX.UI.Menu.CloseAll()
                                ESX.UI.Menu.Open(
                ...
                if OwnedHouse.houseId == k then
                    if not parksended then
                    local coords = v['door']
                    found, coords, heading = GetClosestVehicleNodeWithHeading(coords.x, coords.y, coords.z, 3.0, 100.0, 2.5)
                    parksended = true
                    TriggerServerEvent("loaf_housing:HouseSetted", OwnedHouse.houseId , coords)
                    --if found then
...
--
                    ...
                    --            if IsControlJustReleased(0, 38) then
                    --                TriggerEvent("esx_advancedgarage:OpenStoreMenu")
                    --            end
                    --        end
--
                    --    end
            ...
            if IsControlJustReleased(0, 38) and Vdist2(GetEntityCoords(PlayerPedId()), storage) <= 2.0 then
                    ...
                    'default', GetCurrentResourceName(), 'storage',
                {
                    ...
                    elements = {
                        ...
                        {label = "Ropa", value = 'player_dressing'},
                        {label = "Guardar Outfit Actual", value = 'player_dressing_save'}
                    },
                },
                function(data, menu)
                    ...
                    elseif data.current.value == 'player_dressing_save' then
                        ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'Nombre',{
                            title = 'Nombre del Outift'
                            },
                            function(data2, menu)
                                TriggerEvent('skinchanger:getSkin', function(skin)
                                    TriggerServerEvent('esx_clotheshop:saveOutfit', data2.value, skin)
                                    ESX.ShowNotification("has salvado tu Outfit")
                                end)
                            end,
                            function(data, menu)
                                menu.close()
                        ...
                        ESX.TriggerServerCallback('loaf_housing:hasGuests', function(has)
                            if not has then
                                ...
                                function(data2, menu2)
                                    ...
                                    if furniture[OwnedHouse['furniture'][data2.current.value]['object']] ~= nil then
                        ...
                        ESX.TriggerServerCallback('loaf_housing:hasGuests', function(has)
                            if not has then
                                ...
                                function(data2, menu2)
                                    ...
                                    local object = CreateObject(model, GetOffsetFromEntityInWorldCoords(house, Config.Offsets[prop]['spawn_furniture']), false, false, false)
                                    finalCoords = GetOffsetFromEntityInWorldCoords(house, Config.Offsets[prop]['spawn_furniture'])
                                    ...
                                    while true do
                                        ...
                                        elseif IsControlPressed(0, 215) then
                                            ...
                                            if furniture[data2.current.value]['amount'] > 1 then
                                                ...
                                                furniture = {}
                                                --print(json.encode(oldFurniture))
...
RegisterNetEvent("loaf_housing:setPark")
AddEventHandler("loaf_housing:setPark", function(state) 
    parksended = state
end)
...
AddEventHandler('loaf_housing:setHouse', function(house, purchasedHouses)
    ...
    local found = false
    for k, v in pairs(Config.Houses) do
        --print(json.encode(purchasedHouses))
        found = false
        if k ~= OwnedHouse.houseId then
            for j,i in pairs(purchasedHouses) do
                if k == i then
                    found = true
                end
            end
        ...
        if not found then
            ...
            AvailableHouses[k] = nil
        end
    end
    print("availables:"..json.encode({AvailableHouses[4]}))
    for k, v in pairs(Config.Houses) do
        if OwnedHouse.houseId == k then
...
function ScreenToWorld(flags, toIgnore)
    ...
    local rayHandle = StartShapeTestRay(cam3DPos, direction, flags, toIgnore, 4)
