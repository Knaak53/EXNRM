...
function ShowNoPhoneWarning ()
  ESX.ShowNotification('No dispones de un telefono movil')
end
...
AddEventHandler('esx:onPlayerDeath', function()
  if menuIsOpen then
    ...
    TriggerEvent("esx_ladderhud:hideRadar", false)
...
RegisterKeyMapping("openPhone", "Mostrar/Ocultar teléfono", 'keyboard', "F1")
...
RegisterNetEvent('gcPhone:use_phonebox')
AddEventHandler('gcPhone:use_phonebox', function(entity)
   	local phoneCoords = GetEntityCoords(entity)
   	local number
    if key == -1058868155 then
      number = ('0%.2s-%.2s%.2s'):format(math.abs(phoneCoords.x*100), math.abs(phoneCoords.y * 100), math.abs(phoneCoords.z *100)) .. " CATALÁ"
    else
...
--[[Citizen.CreateThread(function()
	...
	for _, key in pairs({'p_phonebox_01b_s', 'p_phonebox_02_s', 'prop_phonebox_01a', 'prop_phonebox_01b', 'prop_phonebox_01c', 'prop_phonebox_02', 'prop_phonebox_03', 'prop_phonebox_04', -1058868155}) do
	  local closestPhone = GetClosestObjectOfType(coords.x, coords.y, coords.z, 5.0, key, false)
...
Citizen.CreateThread(function ()
  ...
  while true do 
    ...
    --if not inRangeToActivePhone and not currentPlaySound then
    --	Citizen.Wait(750)
    --end
    Citizen.Wait(0)
  end
...
function TooglePhone() 
  ...
  if menuIsOpen == true then 
    ...
    TriggerEvent("esx_ladderhud:showRadar", false)
    SetBigmapActive(1,0)
  else
    PhonePlayOut()
    TriggerEvent('gcPhone:setMenuStatus', false)
...
Citizen.CreateThread(function()
  for k, v in pairs(Config.phoneshopPed) do
      ...
      if not DoesEntityExist(pedEntities[i]) then
          pedEntities[k] = CreatePed(4, hash, v.coords.x, v.coords.y, v.coords.z, v.heading)
          ...
          TaskStartScenarioInPlace(pedEntities[i], v.animation, 0, true)
      end
    end
    SetModelAsNoLongerNeeded(hash)
end)

RegisterNetEvent('gcphone:openPhoneShop')
AddEventHandler('gcphone:openPhoneShop', function()
    TriggerEvent('generic_shop_creator:createShop', Config.shopItems, "Tienda de Moviles")
end)
