...
AddEventHandler('gcPhone:sendMessage', function(number, message)
    ...
    print(json.encode(PhoneNumbers[number]))
    if PhoneNumbers[number] ~= nil then
...
--
    if PhoneNumbers[xPlayer.getJob().name] ~= nil then
      TriggerEvent('esx_addons_gcphone:addSource', xPlayer.getJob().name, source)
    end
  --end)
...
AddEventHandler('esx:playerDropped', function(source)
  ...
  local xPlayer = ESX.GetPlayerFromId(source)
  if PhoneNumbers[xPlayer.getJob().name] ~= nil then
    TriggerEvent('esx_addons_gcphone:removeSource', xPlayer.getJob().name, source)
  end
end)
...
function getPhoneNumber (source, callback) 
  ...
  if xPlayer == nil then
    ...
    callback(xPlayer.get("phone_number"))
  end
  --MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier',{
  --  ['@identifier'] = xPlayer.identifier
  --}, function(result)
