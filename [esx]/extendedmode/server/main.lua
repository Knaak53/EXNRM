...
function onPlayerJoined(playerId)
	...
	if identifier then
		if ESX.GetPlayerFromIdentifier(identifier) and not Config.EnableDebug then
			...
			db.retrieveUser(identifier, function(result)
				print("obtenido")
				if result then
					loadESXPlayer(identifier, playerId)
				else
					print("user not exist, creating...")
					db.createUser(GetHashKey(identifier),identifier, license, function()
						print("user created, loading...")
						loadESXPlayer(identifier, playerId)
...
function loadESXPlayer(identifier, playerId)
	...
	db.retrieveUser(identifier,function(result)
		...
		if ESX.ActiveWorkers[userData.job.name] == nil then
			ESX.ActiveWorkers[userData.job.name] = {}
		end
		
		table.insert(ESX.ActiveWorkers[userData.job.name], playerId)

		if gradeObject.skin_male then userData.job.skin_male = gradeObject.skin_male end
		if gradeObject.skin_female then userData.job.skin_female = gradeObject.skin_female end
		...
		if result.loadout and result.loadout ~= '' then
			...
			for name,weapon in pairs(loadout) do
				...
				local wweight = ESX.GetWeaponWeight(name)
				if label then
					...
					userData.weight = userData.weight + wweight
				end
			end
		end
		...
		if result.variables ~= nil then
			xPlayer.setPlayerVariables(result.variables)

			if result.variables["vehicles"] then
				local playerVehs = result.variables["vehicles"]
				if playerVehs then
					for k,v in pairs(playerVehs) do
						if ESX.Vehicles[k] then
							ESX.Vehicles[k].setOwnerId(playerId)
						end
					end
				end
			end
		...
		--licencias para la Alpha 
		if xPlayer.get("licenses") == nil then
			TriggerEvent("esx_license:addLicense", playerId, "weapon", function() end)
			TriggerEvent("esx_license:addLicense", playerId, "drive", function() end)
		end
		...
		for k, v in ipairs(GetPlayerIdentifiers(playerId)) do
			if string.sub(v, 1, string.len("discord:")) == "discord:" then
				if xPlayer.get("discordIdentifier") ~= nil then
					if xPlayer.get("discordIdentifier") ~= v then
						DropPlayer(playerId, "Tu cuenta de Discord difiere de la cuenta original de Discord registrada")
					end
				else
					xPlayer.set("discordIdentifier", v)
				end
...
AddEventHandler('esx:updateWeaponAmmo', function(weaponName, ammoCount)
	...
	if xPlayer then
		if xPlayer.hasWeapon(weaponName) then
			...
			xPlayer.triggerEvent('esx:removeWeapon', weaponName)
		end
	end
end)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				sourceXPlayer.showNotification(_U('gave_item', itemCount, sourceItem.label, targetXPlayer.getName()))
				targetXPlayer.showNotification(_U('received_item', itemCount, sourceItem.label, sourceXPlayer.getName()))
			else
...
AddEventHandler('esx:removeInventoryItem', function(type, itemName, itemCount, prop)
	local playerId = source
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if type == 'item_standard' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				ESX.CreatePickup('item_standard', itemName, itemCount, pickupLabel, playerId,nil,nil, prop)
			end
		end
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
	...
	elseif type == 'item_weapon' then
		...
		if xPlayer.hasWeapon(itemName) then
			...
			local components = {table.unpack(weapon.components)}


			
			ESX.CreatePickup('item_weapon', itemName, weapon.ammo, pickupLabel, playerId, components, weapon.tintIndex, nil, weapon.serial)
...
AddEventHandler('esx:onPickup', function(id)
	print("pickup id:".. id)
	local pickup, xPlayer, success = ESX.Pickups[id], ESX.GetPlayerFromId(source), false
	...
	if pickup then
		...
		elseif pickup.type == 'item_weapon' then
			...
			elseif xPlayer.canCarryWeapon(pickup.name) then
				success = true
				xPlayer.addWeapon(pickup.name, pickup.count, pickup.serial)
				xPlayer.setWeaponTint(pickup.name, pickup.tintIndex)

				print("COMPONENTS: "..json.encode(pickup.components))
...
RegisterServerEvent("esx:setVehicleProperties")
AddEventHandler("esx:setVehicleProperties", function(vehNetId, props) 
	local xVehicle = ESX.GetVehicleByNetId(vehNetId)
	xVehicle.setVehicleProperties(props)
end)


--RegisterServerEvent("esx:setVehicleDoorState")
--AddEventHandler("esx:setVehicleDoorState", function(vehNetId, doorState) 
--	local xVehicle = ESX.GetVehicleByNetId(vehNetId)
--	xVehicle.setVehicleDoorState(doorState)
--end)
