...
ESX.GetWeaponLabel = function(weaponName, hashed)
	if not hashed then
		weaponName = string.upper(weaponName)
	end
	...
	for k,v in pairs(Config.Weapons) do
		if hashed then
			print(GetHashKey(v.name) .. " =? " .. weaponName)
			if GetHashKey(v.name) == weaponName then
				return v.label
			end
		else
			if v.name == weaponName then
...
ESX.GetWeaponWeight = function(weaponName)
	weaponName = string.upper(weaponName)
...
ESX.GetWeaponsByAmmo = function(ammoHash)
	local weapons = {}

	for k,v in pairs(Config.Weapons) do
		if v.ammo ~= nil then
			if v.ammo.hash == ammoHash then
