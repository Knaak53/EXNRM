...
ESX.ShowNotification = function(msg, flash, saveToBrief, hudColorIndex)
	TriggerEvent("esx_custom_messages:showTextMessage", msg)
...
ESX.UI.HUD.toggleInventoryNotification = function(toggle)
	SendNUIMessage({
		action  = 'toggle_inventoryNotification',
		toggle = toggle
	})
end
...
ESX.UI.ShowInventoryItemNotification = function(add, item, count, label)
	local message
	local movement
	if add then
		message = "Has recibido ~g~x" .. count .. "~b~ " .. label
		movement = "add"
	else
		message = "Se ha eliminado ~g~x" .. count .. "~b~ " .. label .. "~w~ del inventario"
		movement = "remove"
	end
	TriggerEvent('esx:inventoryUpdate', item, add)
	TriggerEvent('esx_custom_messages:showInventoryTextMessage', message, item, movement)
	--[[SendNUIMessage({
...
ExM.Game.PlayAnim = function(animDict, animName, upperbodyOnly, duration)
	...
	TaskPlayAnim(GetPlayerPed(-1), animDict, animName, 2.0, 2.0, runTime, flags, 0, false, false, false)
...
ESX.Game.SpawnVehicle = function(model, coords, heading, cb, networked)
	...
	--TriggerServerEvent("esx:spawnserversidevehicle", model, coords)
...
ESX.Game.SpawnVehiclePedDriven = function(model,spawncoords,heading,drivecoords,pedmodel,velocity,plate,cb,networked,driveType)
	local vector = type(spawncoords) == "vector3" and spawncoords or vec(spawncoords.x, spawncoords.y, spawncoords.z)
	local vectorDrive = type(drivecoords) == "vector3" and drivecoords or vec(drivecoords.x, drivecoords.y, drivecoords.z)
	if driveType == nil then
		driveType = 786603
	end
	networked = networked == true or false
	if networked then
	...
	CreateThread(function()
		...
		local vehicle = CreateVehicle(model, vector.xyz, heading, networked, false)
		local id = NetworkGetNetworkIdFromEntity(vehicle)
		local pedDriver = CreatePedInsideVehicle(vehicle, 4, GetHashKey(pedmodel), -1,true,false)
		...
		if GetDistanceBetweenCoords(spawncoords,drivecoords,true) < 200 then
			TaskVehicleDriveToCoord(pedDriver, vehicle, vectorDrive.x, vectorDrive.y, vectorDrive.z, velocity/3.6, 1.0, GetHashKey(model), driveType, 1.0, 1)
		else
			TaskVehicleDriveToCoordLongrange(pedDriver, vehicle, vectorDrive.x,  vectorDrive.y, vectorDrive.z, velocity/3.6,driveType, 1.0)
		end
		if cb then
			cb(vehicle,pedDriver)
		end
	end)
end
...
ESX.Game.GetPlayerServerIdFromPlayerEntity = function(entity)
	local players, playerId = ESX.Game.GetPlayers(), PlayerId()
	for i=1, #players, 1 do
		local target = GetPlayerPed(players[i])
		if players[i] ~= playerId and target == entity then
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	if NetworkGetEntityIsNetworked(vehicle) then
		if DoesEntityExist(vehicle) then
			TriggerServerEvent("esx:setVehicleProperties", NetworkGetNetworkIdFromEntity(vehicle), props)
		end
	else
		ESX.Game.SetVehiclePropertiesToThisVehicle(vehicle, props)
	end
end

ESX.Game.SetVehiclePropertiesToThisVehicle = function(vehicle, props)
	print("Vehicle: "..vehicle.. " props: "..json.encode(props))
	if DoesEntityExist(vehicle) then
		local colorPrimary, colorSecondary = GetVehicleColours(vehicle)
		local pearlescentColor, wheelColor = GetVehicleExtraColours(vehicle)
...
ESX.PlaySound = function(soundFile,soundVolume)
	SendNUIMessage({
...
RegisterNetEvent('esx:PlaySoundOnPlayer')
AddEventHandler('esx:PlaySoundOnPlayer',function(soundFile,soundVolume)
	ESX.PlaySound(soundFile,soundVolume)
end)

RegisterNetEvent('esx:PlayWithinDistance')
AddEventHandler('esx:PlayWithinDistance',function(playerNetId,maxDistance,soundFile,soundVolume)
...
ESX.CreateBlip = function(coords, sprite, colour, scale, text,resource)
	...
	if ESX.Blips[resource] == nil then 
		ESX.Blips[resource] = {}
	end
	ESX.Blips[resource][blip] = blip
	return blip
end

ESX.DeleteBlip = function(blip,resource)
	RemoveBlip(ESX.Blips[resource][blip])
	ESX.Blips[resource][blip] = nil
end

--Borrar los blips si algun resource se reinicia
AddEventHandler('onResourceStop', function(resource)
	if ESX.Blips[resource] ~= nil then
		for k,v in pairs(ESX.Blips[resource]) do 
			print(v)
			RemoveBlip(v)
		end
		ESX.Blips[resource] = nil
	end
	if ESX.Text3D[resource] ~= nil then
		ESX.Text3D[resource] = nil
	end
end)

ESX.GetResourceBlips = function(resource)
	if ESX.Blips[resource] ~= nil then
		return ESX.Blips[resource]
	else
		return false
	end
end
...
ESX.Markers.Add = function(mType, mPos, red, green, blue, alpha, rangeToShow, bobUpAndDown, mScale, mRot, mDir, faceCamera, textureDict, textureName)
	...
	local markerData = {
		range = rangeToShow,
		type = mType,
		pos = vector3(mPos.x, mPos.y, mPos.z),
		dir = mDir,
		rot = mRot,
		scale = mScale,
		r = red,
...
--Colour must be like this {r=12,g=23,b=123,a=123}
--Backgroudn color like colour
--textColour nil for white text and table like {r=12,g=23,b=123,a=123} for colour
--if entity text will display on entity
-- scale debe ser entre 0.0 y 1.0
ESX.Game.Utils.Draw3DText = function(coords,text,scale,distance,script,textColour,drawBackground,backgroundColor)
	if ESX.Text3D[script] == nil then
		ESX.Text3D[script] = {}
	end
	if coords ~= nil or text ~= nil or scale ~= nil or script ~= nil then
		table.insert(ESX.Text3D[script],{
			coords = coords,
			text = text,
			scale = scale,
			distance = distance,
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		for k,v in pairs(ESX.Text3D) do
			for k2,v2 in pairs(v) do
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),v2.coords,true) <= v2.distance then
					local coords = v2.coords + vector3(0.0, 0.0, 1.2)
					local onScreen,_x,_y=World3dToScreen2d(v2.coords.x, v2.coords.y, v2.coords.z)
					local px,py,pz=table.unpack(GetGameplayCamCoords())

					SetTextScale(v2.scale, v2.scale)
					SetTextFont(4)
					SetTextProportional(1)
					if v2.colour ~= nil then
						SetTextColour(v2.colour.r, v2.colour.g, v2.colour.b, v2.colour.a)
					else
						SetTextColour(255, 255, 255, 215)
					end
					SetTextEntry("STRING")
					SetTextCentre(1)
					AddTextComponentString(v2.text)
					DrawText(_x,_y)
					if v2.drawBackground == true then
						local factor = (string.len(v2.text)) / 370
						if v2.backgroundColor then
							DrawRect(_x,_y+0.0125, 0.015+ factor, 0.03, v2.backgroundColor.r, v2.backgroundColor.g, v2.backgroundColor.b, v2.backgroundColor.a)
						else
...
RegisterNUICallback('submit',function(data)
	inputText = data.stringtext
end)

ESX.Input = function(title,placeholder,cb)
