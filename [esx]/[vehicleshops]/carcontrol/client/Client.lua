...
ctrl.update = function()    
    ...
    while true do 
        ...
        if inVeh then        
            if IsControlJustReleased(1, 99) then          
            ...
            if GetPedInVehicleSeat(vehicle, -1) == playerPed and IsPedInAnyVehicle(playerPed, false) then
                ...
                if IsControlJustReleased(0,246) then
                    if controlEnabled then
                        maxSpeed = GetVehicleHandlingFloat(vehicle,"CHandlingData","fInitialDriveMaxFlatVel")
                        SetEntityMaxSpeed(vehicle, maxSpeed)
                        showHelpNotification("Velocidad de crucero desactivada")
                        controlEnabled = false
                    else
                        cruise = GetEntitySpeed(vehicle)
                        SetEntityMaxSpeed(vehicle, cruise)
                        cruise = math.floor(cruise * 3.6 + 0.5)
                        showHelpNotification("Velocidad de crucero establecida en "..cruise.." km/h.")
                        controlEnabled = true
                    end
                end
            end
