...
local function repairAnim(ped, coords, veh)
	local dict
...
local function mechanicRepair(vehicle)
		--SetVehicleUndriveable(vehicle,true)
		FreezeEntityPosition(vehicle, true)
		print("lets")
		ESX.Streaming.RequestModel(GetHashKey("mp_m_waremech_01"))
		local vehicle = vehicle
		local coords = GetEntityCoords(vehicle)
		local heading = GetEntityHeading(vehicle)
		local coords = GetObjectOffsetFromCoords(coords, heading, 0.0, -8.0, 0.0)
		local mechanicPed = CreatePed(5, GetHashKey("mp_m_waremech_01"), coords.x,coords.y, coords.z, heading, true, true)
		
		local boneVehicleIndex = GetEntityBoneIndexByName(vehicle, "engine")
		
		local bonecoords = GetWorldPositionOfEntityBone(vehicle, boneVehicleIndex)
		local repairCoords = GetObjectOffsetFromCoords(bonecoords, heading, 1.5, 0.0, 0.0)
		print("go: "..repairCoords)
		Wait(2000)
		print("go: yeh") 
		TaskGoToCoordAnyMeans(mechanicPed, repairCoords.x, repairCoords.y, repairCoords.z, 1.5, 0, 0, 786603, -1082130432)
		Wait(6000)
		SetEntityCoords(mechanicPed, repairCoords.x, repairCoords.y, repairCoords.z)
		SetEntityHeading(mechanicPed, heading + 90)
		repairAnim(mechanicPed,repairCoords,vehicle)
		...
		notification("~g~El mecánico ha reparado tu vehiculo")
		mechanicRepairing = false
		--SetVehicleUndriveable(vehicle,false)
		FreezeEntityPosition(vehicle, false)
		Wait(10000)
		DeletePed(mechanicPed)
		TriggerServerEvent("rea:repairvehicle")
end
...
AddEventHandler('iens:repair', function()
	...
	if IsNearMechanic() then
		if not mechanicRepairing then
			ESX.TriggerServerCallback('Player:checkMoney', function(haveMoney)
				if haveMoney then
					mechanicRepairing = true
					mechanicRepair(vehicle)
				else
					ESX.ShowNotification("No tienes suficiente dinero (1500)",true)
				end
			end)
		else
			ESX.ShowNotification("El mecánico ya está reparando tu vehiculo",true)
		end
		return
	end
	if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
		if GetVehicleOilLevel(vehicle) > 0 then
			...
			notification("~g~" .. repairCfg.fixMessages[fixMessagePos] .. ", Ahora dirígete hacia un mecánico!")
			fixMessagePos = fixMessagePos + 1
			if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
		else
			notification("~r~Tu vehiculo está demasiado dañado. No puedes repararlo! Llama a un mecánico")
		end
	else
...
Citizen.CreateThread(function() 
	--Citizen.CreateThread(function() 
		...
		for _,item in pairs(repairCfg.mechanics) do	
			ESX.Game.Utils.Draw3DText(vector3(item.x,item.y,item.z),"Escribe /repair para reparar tu vehiculo", 0.3, 10,GetCurrentResourceName(),nil,true , {r=0,g=100,b=200,a=150})
		end
	--end)
end)
