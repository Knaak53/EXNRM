...
local myCar = {}
local noVehiclesNear = "No hay ningun vehiculo frente a ti"
local currentTunningCar = 0
...
AddEventHandler('esx_lscustom:installMod', function()
	if DoesEntityExist(currentTunningCar) then
		myCar = ESX.Game.GetVehicleProperties(currentTunningCar)
		TriggerServerEvent('esx_lscustom:refreshOwnedVehicle', myCar)	
	end	
end)

RegisterNetEvent('esx_lscustom:cancelInstallMod')
AddEventHandler('esx_lscustom:cancelInstallMod', function(cancelType)
	local returnPoint
	if cancelType == "fake" then
		returnPoint = fakeCar
	else
		returnPoint = myCar
	end
	if DoesEntityExist(currentTunningCar) then
		ESX.Game.SetVehicleProperties(currentTunningCar, returnPoint)
		if not (myCar.modTurbo) then
			ToggleVehicleMod(currentTunningCar,  18, false)
		end
		if not (myCar.modXenon) then
			ToggleVehicleMod(currentTunningCar,  22, false)
		end
		if not (myCar.windowTint) then
			SetVehicleWindowTint(currentTunningCar, 0)
		end	
	end	
end)

RegisterNetEvent('esx_lscustom:endBill')
AddEventHandler('esx_lscustom:endBill', function()
	SendNUIMessage({action = "endBill"})
end)

RegisterNetEvent('esx_lscustom:startBill')
AddEventHandler('esx_lscustom:startBill', function()
	SendNUIMessage({action = "open"})
end)

function OpenFakeLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		local vehicle = currentTunningCar
		...
		if DoesEntityExist(vehicle) then
				...
				if k == data.current.modType or isRimMod then
					...
					if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
						...
						if isRimMod then
							...
							SendNUIMessage({action = "addMod", piecesPrice = price})			
			...
			if not found then
				GetAction(data.current, true, currentTunningCar)
			end
		else
			ESX.ShowNotification(noVehiclesNear)
		end
	end, function(data, menu) -- on cancel
		menu.close()
		if menuTitle == Config.Menus.main.label then
			SendNUIMessage({action = "endBill"})
		end
		
		TriggerEvent('esx_lscustom:cancelInstallMod', "fake")

		local playerPed = PlayerPedId()
		if DoesEntityExist(currentTunningCar) then
			SetVehicleDoorsShut(currentTunningCar, false)

			if parent == nil then
				lsMenuIsShowed = false
				currentTunningCar = nil
				myCar = {}
			end
		end
	end)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		if DoesEntityExist(vehicle) then
			...
			if not found then
				GetAction(data.current, false, vehicle)
			end
		else
			ESX.ShowNotification(noVehiclesNear)
		end
		...
		menu.close()
		TriggerEvent('esx_lscustom:cancelInstallMod', "real")
...
function GetAction(data, fake, getActionVehicle)
	local elements  = {}
	local menuName  = ''
	local menuTitle = ''
	local parent    = nil
	...
	local playerPed = PlayerPedId()
	local vehicle = getActionVehicle
	local currentMods
	...
	if DoesEntityExist(vehicle) then
		...
		if not fake then
			...
			OpenFakeLSMenu(elements, menuName, menuTitle, parent)
		end
	else
		ESX.ShowNotification(noVehiclesNear)
	end
...
RegisterNetEvent('ls_custom:tunning')
AddEventHandler('ls_custom:tunning', function(vehicleEntity)
	if not lsMenuIsShowed and isInLSMarker then
		local vehicle = vehicleEntity
		currentTunningCar = vehicle
		if DoesEntityExist(vehicle) then
			lsMenuIsShowed = true
			myCar = ESX.Game.GetVehicleProperties(vehicle)
			ESX.UI.Menu.CloseAll()
			GetAction({value = 'main'}, false, vehicle)
		end
	else
		ESX.ShowNotification("Debes estar en un taller para eso")
	end
end)

RegisterNetEvent('ls_custom:tunningFake')
AddEventHandler('ls_custom:tunningFake', function()
	if not lsMenuIsShowed and isInLSMarker then
		local vehicle = ESX.Game.GetClosestVehicle(GetEntityCoords(PlayerPedId()))
		currentTunningCar = vehicle
		if DoesEntityExist(vehicle) then
			lsMenuIsShowed = true
			fakeCar = ESX.Game.GetVehicleProperties(vehicle)

			ESX.UI.Menu.CloseAll()
			SendNUIMessage({action = "open"})
			GetAction({value = 'main'}, true, vehicle)	
		else
			ESX.ShowNotification(noVehiclesNear)
		end
	else
