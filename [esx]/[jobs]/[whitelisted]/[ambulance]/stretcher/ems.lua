...
strNames = { 'v_med_bed1', 'v_med_bed2','prop_ld_stret_test'} -- Add more model strings here if you'd like
bedNames = { 'v_med_bed1', 'v_med_bed2', 'v_med_cor_emblmtable'}
local inBed = true
strHashes = {}
...
AddEventHandler("ARPF-EMS:togglestrincar", function(ambulance)
	
	local veh = VehicleInFront()
    local ped = GetPlayerPed(-1)
    local pedCoords = GetEntityCoords(ped)
    local closestObject = GetClosestObjectOfType(pedCoords, 4.0, GetHashKey("prop_ld_stret_test"), false)
    if IsEntityAttachedToAnyVehicle(closestObject) then
    	incar = true
    elseif IsEntityAttachedToEntity(closestObject, veh) then 
    	incar = true
    end
    if not Entity(ambulance).state.stretcherInto then 
        StreachertoCar(ambulance)
        incar = true
    elseif Entity(ambulance).state.stretcherInto then
        incar = false
        StretcheroutCar(ambulance)
    end
    ClearPedTasks(ped)
end)

RegisterNetEvent("moveToBed")
AddEventHandler("moveToBed", function(netId) 
    local ped = PlayerPedId()
    TriggerEvent("unsit", NetToObj(netId))
    DetachEntity(ped)
    ...
    for k,v in pairs(bedNames) do
        local closestObject = GetClosestObjectOfType(pedCoords, 4.0, GetHashKey(v), false)
        if DoesEntityExist(closestObject) then
            ...
            local heading = GetEntityHeading(closestObject) - 180
            SetEntityHeading(ped, heading)
            print("headinG:"..heading)
               if GetDistanceBetweenCoords(pedCoords, sitCoords, true) <= 2.0 then
                   TriggerEvent('sit', closestObject, offset) 
                   break
               end
        end
    end
...
RegisterNetEvent("StreachertoCar")
AddEventHandler("StreachertoCar", function(ambuId, stretId, into) 
    if into then
        StreachertoCarOwned(NetToVeh(ambuId), NetToObj(stretId))
    else
        StretcheroutCarOwned(NetToVeh(ambuId), NetToObj(stretId))
    end
end)

function StreachertoCar(ambulance)
    print("tocarllego antes")
    local veh
    if ambulance then
        veh = ambulance
    else
    ...
    local pedCoords = GetEntityCoords(ped)
    local closestObject = GetClosestObjectOfType(pedCoords, 4.0, GetHashKey("prop_ld_stret_test"), false)
    if DoesEntityExist(closestObject) and (not IsEntityAttached(closestObject) or IsEntityAttachedToAnyVehicle(closestObject)) then
        TriggerServerEvent("StreachertoCar", VehToNet(ambulance), ObjToNet(closestObject), true)
       
    end
end

function StreachertoCarOwned(ambulance, stret)
    print("tocarllego")
    local veh
    if ambulance then
        veh = ambulance
    ...
    local ped = PlayerPedId()
    local closestObject = stret
    if DoesEntityExist(closestObject) then
        if GetVehiclePedIsIn(ped, false) == 0 and DoesEntityExist(veh) and IsEntityAVehicle(veh) then
            print("attacheo")
            AttachEntityToEntity(closestObject, veh, 0.0, 0.0, -2.7, 0.0, 0.0, 0.0, 90.0, false, false, false, false, 2, true)
            --FreezeEntityPosition(closestObject, true)
            TriggerServerEvent("stretIntoAmbulance", NetworkGetNetworkIdFromEntity(ambulance), true)
            --TriggerServerEvent("stretIntoAmbulance", NetworkGetNetworkIdFromEntity(ambulance), false)
...
function StretcheroutCar(ambulance)
    print("llego antes")
    local veh
    if ambulance then
        veh = ambulance
    ...
    local closestObject = GetClosestObjectOfType(pedCoords, 4.0, GetHashKey("prop_ld_stret_test"), false)
    if DoesEntityExist(closestObject) then
        TriggerServerEvent("StreachertoCar", VehToNet(ambulance), ObjToNet(closestObject), false)
    end
end

function StretcheroutCarOwned(ambulance, stret)
    print("llego")
    local veh
    if ambulance then
        veh = ambulance
...
--    LoadModel('prop_ld_stret_test')
--    local str = CreateObject(GetHashKey('prop_ld_stret_test'), GetEntityCoords(PlayerPedId()), true)
--    SetEntityCollision(str, true, true)
--    ActivatePhysics(str)
--    FreezeEntityPosition(str, false)
--end, false)
--
--RegisterCommand('delStr', function(source, args)
--	local object = GetHashKey('prop_ld_stret_test')
...
function delStr() 
    ...
    if myJob == "ambulance" then
        ...
        local x, y, z = table.unpack(GetEntityCoords(pped, true))
        if not GetIsTaskActive(pped) and not IsEntityPlayingAnim(pped,'anim@heists@box_carry@', 'idle', 3) then
            local obj = GetClosestObjectOfType(x, y, z, 3.5, object, false, false, false)
            print("objeto: "..obj)
            local attachedToWhat = GetEntityAttachedTo(obj) and not nil or "none" 
            TriggerServerEvent("stretcher:leaveThisStret", NetworkGetNetworkIdFromEntity(obj))
        end
    end
end
...
--		local ped = PlayerPedId()
--        local pedCoords = GetEntityCoords(ped)
--        local closestObject = GetClosestObjectOfType(pedCoords, 10.0, GetHashKey("prop_ld_stret_test"), false)
--        if DoesEntityExist(closestObject) then
--            local strCoords = GetEntityCoords(closestObject)
...
AddEventHandler("ARPF-EMS:pushstreacherss", function(netId)
        local ped = PlayerPedId()
        local pedCoords = GetEntityCoords(ped)
        print("ID.."..netId.. " Entity: "..NetToObj(netId))
        local closestObject = NetworkGetEntityFromNetworkId(netId)
        if DoesEntityExist(closestObject) then
            local strCoords = GetEntityCoords(closestObject)
            local strVecForward = GetEntityForwardVector(closestObject)
            local sitCoords = (strCoords + strVecForward * - 0.5)
...
AddEventHandler('sit', function(strObject, otheroffset)
  ...
  if IsPedDeadOrDying(playPed) then
    revivePed(playPed)
    if otheroffset then
        local cco = GetEntityCoords(strObject)
        --FreezeEntityPosition(PlayerPedId(), true)
        SetEntityCoords(PlayerPedId(), cco.x + otheroffset.x , cco.y + otheroffset.y, cco.z + otheroffset.z, 0.0, 0.0, 0.0)
        --FreezeEntityPosition(PlayerPedId(), true)
        inBed = true
    else
        AttachEntityToEntity(PlayerPedId(), strObject, 0, 0, 0.0, 1.1, 0.5, 0.0, 180.0, 0.0, false, true, true, false, 2, true)
    end
    print("seeending")
    TriggerServerEvent("playerAttachedToStret", NetworkGetNetworkIdFromEntity(strObject))
    local heading = GetEntityHeading(strObject)
    wasdead = true
    while IsEntityAttachedToEntity(PlayerPedId(), strObject) or inBed do
        Citizen.Wait(5)
  ...
  elseif not IsPedDeadOrDying(playPed) then
    if otheroffset then
        ...
        --FreezeEntityPosition(PlayerPedId(), true)
        inBed = true
        Entity(PlayerPedId()).state.inBed = inBed
        --AttachEntityToEntity(PlayerPedId(), strObject, 0, 0, 0.0, otheroffset.z, 0.0, 0.0, 180.0, 0.0, false, false, true, false, 2, true)
        --Wait(500)
        --DetachEntity(PlayerPedId())
    else
        AttachEntityToEntity(PlayerPedId(), strObject,0, 0, 0.0, 1.425, 0.0, 0.0, 180.0, 0.0, false, true, true, false, 2, true)
    end
    print("seeending")
    TriggerServerEvent("playerAttachedToStret", NetworkGetNetworkIdFromEntity(strObject))
    local heading = GetEntityHeading(strObject)
...
RegisterNetEvent("externalunsit")
AddEventHandler("externalunsit", function(netIdStret) 
    if DoesEntityExist(NetToObj(netIdStret)) then
        TriggerEvent("unsit", NetToObj(netIdStret))
    end
end)
...
AddEventHandler('unsit', function(strObject)   
    if wasdead == true then
        ...
        wasdead = false
        TriggerServerEvent("stretcherOut")
...
function PickUp(strObject)
    ...
    AttachEntityToEntity(strObject, pedid, GetPedBoneIndex(PlayerPedId(),  28422), 0.0, -0.95, -0.90, 195.0, 180.0, 180.0, 0.0, false, false, true, false, 2, true)
