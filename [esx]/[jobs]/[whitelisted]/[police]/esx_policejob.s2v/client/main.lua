...
--desactivar recompensa coches IsOnlinePoliciesMenuActive()
...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'secret_wear' then
			local awaitService
...
function OpenArmoryMenu(station)
	...
	table.insert(elements, {label = "Recargar Munición", value = 'clip'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory', {
		...
		elseif data.current.value == 'clip' then
			TriggerServerEvent("esx_policejob:getClip")
		end
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			...
			if Config.EnableIdentityCard then
				table.insert(elements,{label = _U('identity_card_photo'), value = 'takeidentitycardphoto'})
				table.insert(elements,{label = _U('view_identity_card'), value = 'viewidentitycard'})
			end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'takeidentitycardphoto' then
						TakeIdentityCardPhoto(closestPlayer)
					elseif action == 'viewidentitycard' then
...
function TakeIdentityCardPhoto(player)
	DestroyMobilePhone()
...
RegisterNUICallback('getnewphoto', function(data)
	if data.image then
		TriggerServerEvent('esx_policejob:setIdentityPhoto',GetPlayerServerId(data.player),data.image)
	end
end)
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		if data.validated then
			...
			ESX.ShowNotification(_U('no_identity_card'), true, true, 6)
		end
...
function OpenBodySearchMenu(player)
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		...
		if data.money then
			table.insert(elements, {
				label    = _U('confiscate_inv', data.money, "Dinero Efectivo"),
				value    = "money",
				itemType = 'item_money',
				amount   = data.money
			})
	...
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				local mugshot = ESX.Game.GetPedMugshot(GetPlayerPed(-1))
				TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'police', _U('fine_total', data.current.fineLabel), data.current.amount, mugshot)
			else
...
-- RegisterCommand("testbills", function(source, args, raw)

-- 	TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'police', "XD", 500)

-- 	ESX.TriggerServerCallback('esx_billing:getTargetBills', function(bills)
-- 		print(json.encode(bills))
...
function OpenGetWeaponTypeMenu()
	ESX.TriggerServerCallback('esx_policejob:getArmoryWeaponsType', function(weaponsType)
		local elements = {}

		for k,v in pairs(weaponsType) do
			table.insert(elements, {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_type_weapon', {
			title    = _U('get_weapon_type_menu'),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			menu.close()

			ESX.TriggerServerCallback('esx_policejob:getArmoryWeapons', function(weapons, name)
				OpenGetWeaponMenu(weapons, name)
...
function OpenGetWeaponMenu(weapons,name)
	local elements = {}
	local name = name
	for k,v in ipairs(weapons) do
		local labelw
		for i,j in pairs(v.components) do
			if i == 1 then
				labelw = '[<span style="color:blue;font-weight:bold">'..ESX.GetWeaponComponent(v.name, j).label.."</span>,"
			elseif i == #v.components then
				labelw = labelw .. '[<span style="color:blue;font-weight:bold">'..ESX.GetWeaponComponent(v.name, j).label.."</span>" ..']'
			else
				labelw = labelw .. '[<span style="color:blue;font-weight:bold">'..ESX.GetWeaponComponent(v.name, j).label.."</span>,"
			end
		end
...
function OpenBuyWeaponsMenu()
	...
	for k,v in pairs(Config.AuthorizedWeapons[ESX.PlayerData.job.grade_name]) do
...
function initializePoliceCommands()
	RegisterCommand("atarManos", function() 
		if ESX.PlayerData.job.name == "police" then
			gangHandCuff()
		end
	end, false)
	
	RegisterCommand("mirarInv", function() 
		if ESX.PlayerData.job.name == "police" then
			local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
			if closestPlayer ~= -1 and closestDistance <= 3.0 then
				ExecuteCommand("openinventory "..GetPlayerServerId(closestPlayer))
			end
		end
	end, false)
	
	RegisterKeyMapping('atarManos', 'Atar manos siendo banda', 'keyboard', 'm')
	RegisterKeyMapping('mirarInv', 'Atar manos siendo banda', 'keyboard', '0')
end
