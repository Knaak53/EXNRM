	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		elseif data.current.action == 'garage' then
			...
			ESX.TriggerServerCallback('esx_policejob:retrieveJobVehicles', function(jobVehicles)
				if tablelength(jobVehicles) > 0 then
					...
					for k,v in pairs(jobVehicles) do
						...
						if IsModelInCdimage(model) then
							...
							local label = ('%s - <span style="color:darkgoldenrod;">%s</span>: '):format(vehicleName, v.plate)
							...
							allVehicleProps[v.plate] = {model = model, plate = v.plate}
						end
					end

					if tablelength(garage) > 0 then
