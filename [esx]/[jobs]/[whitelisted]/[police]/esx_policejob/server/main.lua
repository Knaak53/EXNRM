...
RegisterNetEvent("esx_policejob:putzepo")
AddEventHandler("esx_policejob:putzepo", function(plate) 
	local xVehicle = ESX.GetVehicleByPlate(plate)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xVehicle then
		if xPlayer.getJob().name == "police" then
			if xVehicle.isfreezed() then
				xVehicle.freeze(false)
				xPlayer.showNotification('Has quitado el ~r~ZEPO~w~ correctamente')
			else
				xPlayer.showNotification('Has puesto un ~g~ZEPO~w~ correctamente')
				xVehicle.freeze(true)
			end
		end
	else
		xPlayer.showNotification('No se puede poner ~r~un zepo~w~ a este vehiculo')
	end
end)
	...
	if itemType == 'item' then
		...
		if targetItem.count > 0 and targetItem.count <= amount then
			...
			if sourceXPlayer.canCarryItem(itemName, sourceItem.count) then
				...
				sourceXPlayer.showNotification("~r~No puedes llevar tanto peso!")
			end
		else
...
ESX.RegisterServerCallback('esx_policejob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	local authorizedWeapons, selectedWeapon = Config.AuthorizedWeapons[xPlayer.getJob().grade]
...
ESX.RegisterServerCallback('esx_policejob:getCash', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	cb(xPlayer.getMoney())
end)	
...
ESX.RegisterServerCallback('esx_policejob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerClientEvent("esx:showNotification", _source, "El vehiculo debe ser comprado en el concesionario")
