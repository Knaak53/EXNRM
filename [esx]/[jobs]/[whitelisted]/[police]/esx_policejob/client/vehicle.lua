...
DeleteGarage = function()
    if DoesEntityExist(pedEntity2) then
        DeletePed(pedEntity2)
        SetPedAsNoLongerNeeded(pedEntity2)
    end
    if DoesEntityExist(pedEntity3) then
        DeletePed(pedEntity3)
        SetPedAsNoLongerNeeded(pedEntity3)
    end
...
Citizen.CreateThread(function()
    hash = GetHashKey('mp_m_securoguard_01')
    RequestModel(hash)
    while not HasModelLoaded(hash) do
		Wait(0)
	end
    if not DoesEntityExist(pedEntity2) then
        pedEntity2 = CreatePed(4, hash, Config.PoliceStations.LSPD.Vehicles[1].Spawner.x, Config.PoliceStations.LSPD.Vehicles[1].Spawner.y, Config.PoliceStations.LSPD.Vehicles[1].Spawner.z, 230.16)
    ...
    hash = GetHashKey('s_m_y_armymech_01')
    RequestModel(hash)
    while not HasModelLoaded(hash) do
		Wait(0)
	end
    if not DoesEntityExist(pedEntity3) then
        pedEntity3 = CreatePed(4, hash, Config.PoliceStations.LSPD.Helicopters[1].Spawner.x, Config.PoliceStations.LSPD.Helicopters[1].Spawner.y, Config.PoliceStations.LSPD.Helicopters[1].Spawner.z, 157.87)
    ...
    SetModelAsNoLongerNeeded(hash)
    hash = GetHashKey('s_m_y_cop_01')
    RequestModel(hash)
    while not HasModelLoaded(hash) do
		Wait(0)
	end
    if not DoesEntityExist(pedEntity) then
        pedEntity = CreatePed(4, hash, Config.PoliceStations.LSPD.Armories[1].x, Config.PoliceStations.LSPD.Armories[1].y, Config.PoliceStations.LSPD.Armories[1].z, 33.43)
...
RegisterNetEvent('esx_policejob:open_garage_ui')
AddEventHandler('esx_policejob:open_garage_ui', function()
...
RegisterNetEvent('esx_policejob:open_heli_ui')
AddEventHandler('esx_policejob:open_heli_ui', function()
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			local buySpawnCoords = Config.PoliceStations[station][part][partNum].buySpawnCoords
			...
			if authorizedVehicles then
				if #authorizedVehicles > 0 then
					...
					if #shopElements > 0 then
						OpenShopMenu(shopElements, playerCoords, shopCoords, buySpawnCoords)
					else
...
function OpenShopMenu(elements, restoreCoords, shopCoords, buySpawnCoords)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				end, data.current.model, props.plate, buySpawnCoords, props, "police")
			else
				menu2.close()
			end
		end, function(data2, menu2)
