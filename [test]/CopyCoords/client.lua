-- copy coords command
RegisterCommand('ccoords', function(source, args, rawCommand)
...
RegisterCommand('ccoords2', function(source, args, rawCommand)
	local coords = GetEntityCoords(PlayerPedId())
	local heading = GetEntityHeading(PlayerPedId())
	SendNUIMessage({
...
RegisterCommand('ccoords3', function(source, args, rawCommand)
	local coords = GetEntityCoords(PlayerPedId())
	local heading = GetEntityHeading(PlayerPedId())
...
RegisterCommand('ccoords4',function(source,args,raw)
	local coords = GetEntityCoords(PlayerPedId())
	SendNUIMessage({
    ...
    while true do                                           -- Loop it infinitely.
        ...
        if infoOn then                                      -- If the info is on then...
            ...
            if IsPlayerFreeAiming(PlayerId()) then          -- If the player is free-aiming (update texts)...
                ...
                coords = GetEntityCoords(entity)      -- Get the coordinates of the object.
